...
| 2 | "How accurate is the difficulty assessment method? If it is not sound, we have no way of knowing whether the construction method is, either" | **New experiment.** The metric is validated against ground-truth official NYT difficulty levels on 422 puzzles / 1,688 groups: Spearman ρ = −0.449 (p < 1e−83), monotonic per-level means, 70% within-puzzle pairwise ordering accuracy. The paper now explicitly treats the metric as a coarse corpus-level signal and documents where it fails (wordplay categories) | §6, Table 1, Fig. 2; `metrics/validate_difficulty.py`, `results/metric_validation.json` |
...
| 6 | "Seed dataset cited only by a filename (connections.json). What is the source, is it freely available?" | Dataset provenance stated with URL (community-maintained NYT-Connections-Answers archive), scope (422 puzzles from 2023-06-12), license/usage note in the Ethics Statement | §6.2 footnote, Appendix B, Ethics |
...
| 13 | Ll.345 "structured reasoning" conclusion unbacked (one model, one technique) | That claim is removed. The comparison now has n = 1,688 / 6,016 / 200 groups with significance tests, and claims are scoped to what the validated metric supports | §7.2, Table 3 |
